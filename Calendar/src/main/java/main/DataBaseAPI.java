...
public enum DataBaseAPI {
	...
	public void addNewUser(HashMap<String, String> userData) throws Exception {
	    ...
	    log("addNewUser(): user {" + name + ", " + mail + ", " + phone + ", " + pass + "} has been added");
	}
	...
	private User getUserWithEmail(String mail) {
		Query q = em.createQuery("SELECT a FROM User a WHERE a.user_mail=:arg_mail");
	...
	private List<User> getUserWithData(String mail, String pass) {
		...
		Query q = em.createQuery("SELECT a FROM User a WHERE a.user_mail = :arg_mail AND a.user_pass = :arg_pass");
	...
	private boolean userHasFriend(int friendId) {
		Query q = em.createQuery("SELECT u FROM Friend u WHERE u.user_owner = :owner");
	...
	public void addFriend(int friendId) throws Exception {
	    ...
	    log("addFriend(): friend with id " + friendId + " added ( current userId: " + user.getId());
	}
	...
	public List<User> getFriends() throws Exception {
		...
		Query q = em.createQuery("SELECT u FROM User u JOIN Friend f ON u.id = f.user_slave AND f.user_owner = :owner");
	...
	public void createEvent(String title, String desc, Date time, Date delta, Collection<Integer> members) throws Exception {
		if (!isLoggedIn()) {
	...
	public List<Event> getEvents() throws Exception {
		...
		Query q = em.createQuery("SELECT e FROM Event e WHERE e.event_admin = :admin_id");
	...
	public List<User> getEventMembers(int eventId) throws Exception {
		...
		Query q = em.createQuery("SELECT u FROM User u WHERE u.id IN (SELECT u2.member_id FROM Member u2 WHERE u2.event_id = :event)");
